...
export default function EndScreen() {
  ...
  const saveBestScore = useCallback(() => {
    if (totalScore > bestScore) {
      localStorage.setItem(`bestScore-${numOfSongs}`, totalScore.toString());
    }
  }, [bestScore, totalScore, numOfSongs]);
  ...
  function getMessage() {
    ...
    if (totalScore < maxScore * 0.25) return messages.veryBad;
    if (totalScore < maxScore * 0.5) return messages.bad;
    if (totalScore < maxScore * 0.9) return messages.okay;
    if (totalScore < maxScore) return messages.good;
    if (totalScore == maxScore) return messages.perfect;
  }
    ...
    <div className="flex flex-col flex-grow">
      <div className="flex flex-col justify-center m-2 md:m-auto flex-grow">
        ...
        <h4 className="text-2xl bg-black px-4 py-2">
          You scored
          <span className="font-black"> {totalScore} </span>points
        </h4>
        {bestScore && (
          ...
          <ul className="border-b-2 border-black/10 h-64 overflow-y-scroll scrollbar-thin scrollbar-thumb-black/25 scrollbar-track-black/0 cursor-pointer">
            {songs.map((s) => (
              <li
                ...
                onClick={() => window.open(s.external_url, "_blank")}
              >
                <div
